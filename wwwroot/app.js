document.addEventListener('DOMContentLoaded', () => {
    ...
    const initializeApp = async () => {
        ...
        try {
            ...
            console.log("User context received:", currentUser);
            
            console.log("Fetching config settings...");
            config = await apiFetch(`${API_BASE_URL}/config/settings`);
            console.log("Config settings received:", config);
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const handleDisable = async (sam, domain) => {
                if (!confirm(`Are you sure you want to DISABLE the account for ${sam}? This action cannot be undone through this portal.`)) return;
                try {
